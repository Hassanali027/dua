...
<div class="settings-container">
    <div class="header-section" style="margin-bottom: 30px;">
        ...
        <p class="header-subtitle">Create a new product for your store.</p>
    </div>
    ...
    <div class="settings-card">
        <form action="{{ route('admin.product.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" id="name" name="name" class="form-control" required placeholder="e.g. 3 Piece Unstitched Embroidered Lawn Suit" value="{{ old('name') }}">
            </div>

            <div class="form-group">
                <label for="price">Price (Rs.) <span style="color: red;">*</span></label>
                <input type="number" id="price" name="price" class="form-control" step="0.01" min="0" required placeholder="e.g. 17990" value="{{ old('price') }}">
            </div>
            ...
            <div class="form-group" style="display: flex; gap: 20px;">
                            ...
                            @foreach($categories as $category)
                                ...
                                @foreach($category->children as $child)
                                    <label class="dropdown-item" style="padding-left: 30px; font-size: 13px;">
                                        <input type="checkbox" name="categories[]" value="{{ $child->id }}" {{ (is_array(old('categories')) && in_array($child->id, old('categories'))) ? 'checked' : '' }}> {{ $child->name }}
                                    </label>
                                    @foreach($child->children as $grandchild)
                                        <label class="dropdown-item" style="padding-left: 50px; font-size: 12px; color: #6b7280;">
                                            <input type="checkbox" name="categories[]" value="{{ $grandchild->id }}" {{ (is_array(old('categories')) && in_array($grandchild->id, old('categories'))) ? 'checked' : '' }}> {{ $grandchild->name }}
                                        </label>
                                    @endforeach
                                @endforeach
                            @endforeach
                ...
                <div style="flex: 1;">
                    ...
                    <input type="text" id="season" name="season" class="form-control" placeholder="e.g. Summer, Festive" value="{{ old('season') }}">
                </div>
            </div>
            ...
            <div class="form-group" style="display: flex; gap: 20px; align-items: flex-end;">
                <div style="flex: 1;">
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                        <input type="checkbox" name="is_on_sale" id="is_on_sale" value="1" {{ old('is_on_sale') ? 'checked' : '' }} style="width: 16px; height: 16px;">
                        Is Product on Sale?
                    </label>
                </div>
                <div style="flex: 1;">
                    <label for="sale_price">Sale Price (Rs.)</label>
                    <input type="number" id="sale_price" name="sale_price" class="form-control" step="0.01" min="0" placeholder="e.g. 15990" value="{{ old('sale_price') }}">
                </div>
            </div>
            ...
            <div class="form-group" style="display: flex; gap: 20px;">
                ...
                <div style="flex: 1;">
                    ...
                    <input type="number" id="quantity" name="quantity" class="form-control" min="0" required value="{{ old('quantity', 0) }}">
                </div>
            </div>

            <div class="form-group">
                <label for="product_detail">Product Detail (Description)</label>
                <textarea id="product_detail" name="product_detail" class="form-control" rows="4" placeholder="Enter product description here...">{{ old('product_detail') }}</textarea>
            </div>
            ...
            <div class="form-group" style="display: flex; gap: 20px; flex-wrap: wrap;">
                <div style="flex: 1; min-width: 150px; margin-bottom: 10px;">
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                        <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }} style="width: 16px; height: 16px;">
                        Featured
                    </label>
                </div>
                <div style="flex: 1; min-width: 150px; margin-bottom: 10px;">
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                        <input type="checkbox" name="is_best_selling" id="is_best_selling" value="1" {{ old('is_best_selling') ? 'checked' : '' }} style="width: 16px; height: 16px;">
                        Best Selling
                    </label>
                </div>
                <div style="flex: 1; min-width: 150px; margin-bottom: 10px;">
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                        <input type="checkbox" name="is_most_in_demand" id="is_most_in_demand" value="1" {{ old('is_most_in_demand') ? 'checked' : '' }} style="width: 16px; height: 16px;">
                        Most In Demand
                    </label>
                </div>
                <div style="flex: 1; min-width: 150px; margin-bottom: 10px;">
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                        <input type="checkbox" name="is_new_arrival" id="is_new_arrival" value="1" {{ old('is_new_arrival') ? 'checked' : '' }} style="width: 16px; height: 16px;">
                        New Arrival
                    </label>
                </div>
                <div style="flex: 1; min-width: 150px; margin-bottom: 10px;">
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                        <input type="checkbox" name="is_bridal_party_wear" id="is_bridal_party_wear" value="1" {{ old('is_bridal_party_wear') ? 'checked' : '' }} style="width: 16px; height: 16px;">
                        Bridal & Party Wear
                    </label>
                </div>
                <div style="flex: 1; min-width: 150px; margin-bottom: 10px;">
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                        <input type="checkbox" name="show_in_navbar" id="show_in_navbar" value="1" {{ old('show_in_navbar') ? 'checked' : '' }} style="width: 16px; height: 16px;">
            ...
            <div class="form-group" style="display: flex; gap: 20px;">
                ...
                <div style="flex: 1;">
                    ...
                    <small style="color: #6b7280; font-size: 12px; margin-top: 5px; display: block;">You can select multiple images for the gallery.</small>
                </div>
            </div>
            ...
            <div class="form-group" style="display: flex; gap: 20px;">
                <div style="flex: 1;">
                    ...
                    <textarea id="product_details" name="product_details" class="form-control" rows="3" placeholder="Custom product details...">{{ old('product_details') }}</textarea>
                </div>
                <div style="flex: 1;">
                    <label for="product_care">Product Care (Accordion)</label>
                    <textarea id="product_care" name="product_care" class="form-control" rows="3" placeholder="Custom product care instructions...">{{ old('product_care') }}</textarea>
                </div>
            </div>
            ...
            <div class="form-group" style="display: flex; gap: 20px;">
                <div style="flex: 1;">
                    ...
                    <textarea id="shipping" name="shipping" class="form-control" rows="3" placeholder="Custom shipping details...">{{ old('shipping') }}</textarea>
                </div>
                <div style="flex: 1;">
                    <label for="return_exchange">Return & Exchange (Accordion)</label>
                    <textarea id="return_exchange" name="return_exchange" class="form-control" rows="3" placeholder="Custom return & exchange policy...">{{ old('return_exchange') }}</textarea>
                </div>
            </div>
